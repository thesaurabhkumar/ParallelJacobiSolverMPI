...
double norm (int n, int m, double * V[]) {
    ...
    //nrm = my_norm;
    // INSERT CODE HERE
    MPI_Allreduce (&my_norm,&nrm,1,MPI_DOUBLE,MPI_MAX,MPI_COMM_WORLD);
    return nrm;
}
// -----------------------------------------------------------------------------
...
void update_boundary (int n, int m, double * U[]) {
	...
	if (my_right != NO_ONE) {
	    ...
	    for(int i=1;i<m+1;++i){
            ...
            MPI_Send(buf,m+1,MPI_DOUBLE,my_right,0,MPI_COMM_WORLD);
            MPI_Recv(buf,m+1,MPI_DOUBLE,my_right,0,MPI_COMM_WORLD,&status);
            for(int i=1;i<m+1;++i){
                U[n+1][i]=buf[i];
            }
	} else
	...
	if (my_left != NO_ONE) {
	    // INSERT CODE HERE
            ...
            MPI_Send(buf,m+1,MPI_DOUBLE,my_left,0,MPI_COMM_WORLD);
     	    MPI_Recv(buf,m+1,MPI_DOUBLE,my_left,0,MPI_COMM_WORLD,&status);
     	    for(int i=1;i<m+1;++i){
		U[0][i]=buf[i];
	    }
	} else
	...
	if (my_above != NO_ONE) {
	    ...
	    for(int i=1;i<n+1;++i){
            ...
            MPI_Send(buf,n+1,MPI_DOUBLE,my_above,0,MPI_COMM_WORLD);
     	    MPI_Recv(buf,n+1,MPI_DOUBLE,my_above,0,MPI_COMM_WORLD,&status);
     	    for(int i=1;i<n+1;++i){
                U[i][m+1]=buf[i];
	    }
	} else
	...
	if (my_below != NO_ONE) {
	    // INSERT CODE HERE
            ...
            MPI_Send(buf,n+1,MPI_DOUBLE,my_below,0,MPI_COMM_WORLD);
            MPI_Recv(buf,n+1,MPI_DOUBLE,my_below,0,MPI_COMM_WORLD,&status);
            for(int i=1;i<n+1;++i){
                U[i][0]=buf[i];
	    }
	} else
...
void jacobi() {
    ...
    if ((verbose > 0) && (my_id == 0))
	printf("Exiting jacobi; iterations = %d, error norm = %e\n", iter, nrm);
}
// -----------------------------------------------------------------------------
// Main Program
